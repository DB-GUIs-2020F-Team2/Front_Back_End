...
app.get('/order/:OrderID', async (req, res) => {
  connection.query('SELECT * FROM `team2`.`Orders` WHERE `Orders`.`OrderID` = ?',
...
app.get('/order/cost/:OrderID', async (req, res) => {
  connection.query('SELECT *, SUM(IF(p.IsDiscount, p.DiscountPrice, p.CurrentPrice) * op.Amount) AS cost FROM Orders o INNER JOIN Order_Product op ON op.OrderID = o.OrderID INNER JOIN Product p ON p.ProductID = op.ProductID WHERE o.OrderID = ? GROUP BY o.OrderID;',
  [req.params.OrderID], function (err, rows, fields) {
    if (err) {
      logger.error("Error while executing Query: \n", err);
      res.status(400).json({
...
app.get('/orders/before/:ExpireDate', async (req, res) => {
  connection.query(`SELECT * FROM Orders WHERE datediff(Orders.ExpireDate, ?) < 0 ORDER BY Orders.ApplyDate;`,
   [req.params.ExpireDate], function (err, rows, fields) {
    if (err) {
      logger.error("Error while executing Query: \n", err);
      res.status(400).json({
...
app.get('/product/:ProductID', function (req, res) {
  ...
  [req.params.ProductID], function (err, rows, fields) {
    if (err) {
      logger.error("Error while executing Query: \n", err);
      res.status(400).json({
        "data": [],
...
app.get('/order_product/order/:OrderID', async (req, res) =>{
...
app.get('/order_product/product/:ProductID', async (req, res) =>{
